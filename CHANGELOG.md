# **Smart Timetable App (JustTrain) \- 変更履歴**
...
* **機能改善**: データ取得エラー時に表示されるメッセージエリアに「再読み込み」ボタンを追加。API設定画面を開かずに即座にリトライが可能に（CORSプロキシの認証更新後などに便利）。

## **\[5.3.0\] \- 2026-02-02 (Stable Release 6\)**

### **Added**

* **UI改善**: データ読み込み中（ロード中）の状態を視覚的に伝えるため、カード形状のスケルトンローディングアニメーションを追加。ユーザー体験の向上。

## **\[5.2.2\] \- 2026-02-01 (Change Departure Label)**

### **Changed**

* **UI文言変更**: オフィスモードの時刻ラベルを「出発」から「退勤」に変更し、電車発車時刻との混同を防止。
